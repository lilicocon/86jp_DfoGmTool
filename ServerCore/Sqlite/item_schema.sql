...
CREATE INDEX IF NOT EXISTS idx_mailbox_messages_receiver_created ON mailbox_messages(receiver_character_id, created_at);
CREATE INDEX IF NOT EXISTS idx_mailbox_messages_sender_created ON mailbox_messages(sender_character_id, created_at);
-- 与服务端 MigrateMailboxItemCore 一致：过期扫描按 unlimited_flag，而非 mail_type。
CREATE INDEX IF NOT EXISTS idx_mailbox_messages_expiry ON mailbox_messages(unlimited_flag, expire_at, message_id);
CREATE UNIQUE INDEX IF NOT EXISTS ux_mailbox_messages_idempotency ON mailbox_messages(sender_character_id, idempotency_key) WHERE idempotency_key IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_mailbox_recipients_character_folder ON mailbox_recipients(character_id, folder, deleted_flag, created_at);
...
CREATE INDEX IF NOT EXISTS idx_mailbox_attachments_message ON mailbox_attachments(message_id, ordinal);
CREATE UNIQUE INDEX IF NOT EXISTS ux_mailbox_attachments_message_ordinal ON mailbox_attachments(message_id, ordinal);
...
CREATE INDEX IF NOT EXISTS idx_mailbox_system_mail_audit_receiver_created ON mailbox_system_mail_audit(receiver_character_id, created_at);
