...
let inventoryAnomalyDetails = [];
let inventoryAnomalyDetailPage = 0;
const INVENTORY_ANOMALY_PAGE_SIZE = 20;
...
function resetInventoryAnomalyState() {
  ...
  inventoryAnomalyDetails = [];
  inventoryAnomalyDetailPage = 0;
...
function renderInventoryAnomalyDetails(details) {
  inventoryAnomalyDetails = Array.isArray(details) ? details : [];
  if (inventoryAnomalyDetailPage * INVENTORY_ANOMALY_PAGE_SIZE >= inventoryAnomalyDetails.length)
    inventoryAnomalyDetailPage = 0;
  paintInventoryAnomalyDetailsPage();
}

function paintInventoryAnomalyDetailsPage() {
  const box = $('#inventory-anomaly-details');
  const pager = $('#inventory-anomaly-pager');
  if (!box) return;
  const rows = inventoryAnomalyDetails;
  if (rows.length === 0) {
  ...
  const pageCount = Math.max(1, Math.ceil(rows.length / INVENTORY_ANOMALY_PAGE_SIZE));
  if (inventoryAnomalyDetailPage >= pageCount)
    inventoryAnomalyDetailPage = Math.max(0, pageCount - 1);
  const start = inventoryAnomalyDetailPage * INVENTORY_ANOMALY_PAGE_SIZE;
  const pageRows = rows.slice(start, start + INVENTORY_ANOMALY_PAGE_SIZE);
  ...
  if (pager && typeof renderTaskPager === 'function') {
    renderTaskPager(pager, inventoryAnomalyDetailPage, pageCount, rows.length, (page) => {
      inventoryAnomalyDetailPage = page;
      paintInventoryAnomalyDetailsPage();
    });
  } else if (pager) {
    pager.innerHTML = '';
